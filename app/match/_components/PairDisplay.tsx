...
export default function PairDisplay({
  ...
  const topName =
    topCourt === "L" ? safeText(pair.left, `${team}-L`) : safeText(pair.right, `${team}-R`);
  const bottomName =
    bottomCourt === "L" ? safeText(pair.left, `${team}-L`) : safeText(pair.right, `${team}-R`);
    ...
    <div className={styles.pairRow}>
      <div className={styles.pairCell}>
        ...
        <div className={styles.pairName}>
          {topName}
          {server === team && serverCourt === topCourt && <span className={styles.dot} />}
        </div>
      </div>
      ...
      <div className={styles.pairCell}>
        ...
        <div className={styles.pairName}>
          {bottomName}
          {server === team && serverCourt === bottomCourt && <span className={styles.dot} />}
        </div>
      </div>
    </div>
  );
